...
def add_archive(identifier, url, title, description, total_size, files_count, metadata_json, server, dir_path):
    conn = get_db()
    max_pos = conn.execute("SELECT COALESCE(MAX(position), -1) FROM archives").fetchone()[0]
    conn.execute(
        ...
        (identifier, url, title, description, total_size, files_count,
         json.dumps(metadata_json), max_pos + 1, time.time(), server, dir_path),
    )
...
def add_archive_files(archive_id, files):
    conn = get_db()
    # Start priority after existing files
